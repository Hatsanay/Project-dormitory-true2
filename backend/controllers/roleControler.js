...
const updateRole = async (req, res) => {
  ...
  try {
    ...
    const [permissionCheck] = await db.promise().query("SELECT * FROM permissions WHERE permission_id = ?", [permission_id]);
    ...
    const updateQuery1 = `
      ...
      WHERE permission_id = ?
    `;
    await db.promise().query(updateQuery1, [permission_name, permission_id]);
